...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const mapSupabaseUser = (user: User | null): AppUser => {
    if (!user) {
      console.warn(
        "mapSupabaseUser called with null user, returning empty user object",
      );
      return {
        id: "",
        email: "",
    ...
    try {
      if (metadata.identities && Array.isArray(metadata.identities)) {
        const googleIdentity = (metadata.identities as Identity[]).find(
          (identity: Identity) =>
            identity &&
    ...
    return {
      ...
      hasCompletedOnboarding:
        Boolean(metadata.has_completed_onboarding) || false,
    };
  };

  // Track if auth service is initialized
  const [isAuthServiceInitialized, setIsAuthServiceInitialized] =
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Unknown error during auth callback",
      };
    }
  };
  ...
  const updateOnboardingStatus = async (
    ...
    try {
      return await supabaseAuthService.updateOnboardingStatus(
        userId,
        hasCompleted,
      );
    } catch (error) {
