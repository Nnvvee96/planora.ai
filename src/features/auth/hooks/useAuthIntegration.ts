...
export const useAuthIntegration = () => {
   ...
   */
  const updateUserProfileAfterAuth = useCallback(async (user: AppUser): Promise<boolean> => {
    try {
      if (!user || !user.id) {
      ...
      if (!profileExists) {
        ...
        return await userProfileService.updateUserProfile(user.id, {
          email: user.email
        });
      }
    } catch (error) {
   ...
   */
  const completeOnboarding = useCallback(async (userId: string, hasCompleted: boolean = true): Promise<boolean> => {
    try {
      if (!userId) {
      ...
      // Update auth record
      const authSuccess = await authService.updateOnboardingStatus(userId, hasCompleted);
      
      // Update profile record
      const profileSuccess = await userProfileService.updateOnboardingStatus(userId, hasCompleted);
      
      return authSuccess && profileSuccess;
    } catch (error) {
