...
export const supabaseAuthService = {
    ...
    try {
      ...
      try {
        ...
        if (!tableCheckError || tableCheckError.code !== "PGRST204") {
          ...
          console.warn(
            "Email change tracking table not found, continuing without tracking",
          );
        }
      } catch (trackingErr) {
      ...
      if (isGoogleUser) {
        if (!password || password.length < 8) {
          throw new Error(
            "A secure password (min 8 characters) is required when changing from Google authentication",
          );
        }

        console.log(
          "Converting Google user to email/password authentication...",
        );

        // Get the redirect URL for email verification
        const redirectTo = supabaseAuthService.getEmailVerificationRedirectUrl(
        ...
        try {
          ...
          console.log(
            "Successfully initiated Google user conversion to email/password via admin API",
          );
          return; // Success, we're done
        } catch (adminErr) {
          // If admin method fails, fall back to standard method
          console.warn(
            "Admin updateUser failed, falling back to standard updateUser:",
            adminErr,
          );
          ...
          console.log(
            "Successfully initiated Google user conversion to email/password via standard API",
          );
          return; // Success, we're done
        }
      } else {
  ...
  verifyEmail: async (token: string): Promise<boolean> => {
    try {
      ...
      try {
        ...
        if (profileError) {
          console.warn(
            "Could not fetch profile during email verification:",
            profileError,
          );
        ...
        if (profileData) {
          ...
          if (pendingEmailChange && user.email !== currentProfileEmail) {
            console.log(
              `Email change verified: ${currentProfileEmail} \u2192 ${user.email}`,
            );
            ...
            if (updateError) {
              console.error(
                "Error updating profile after email verification:",
                updateError,
              );
            ...
            try {
              ...
              if (
                ...
                console.warn(
                  "email_change_tracking table does not exist in database",
                );
                ...
                if (trackingData && trackingData.auth_provider === "google") {
                  ...
                  console.log(
                    "User metadata updated to reflect Google-to-Email conversion",
                  );
                }
              ...
              console.warn(
                "Non-critical error with tracking record:",
                trackingErr,
              );
            ...
            console.log(
              "Regular email verification completed for:",
              user.email,
            );
  ...
  resendVerificationEmail: async (
    ...
    try {
      ...
      if (userId) {
        const result = await supabaseAuthService.sendVerificationCode(
          userId,
          email,
        );
      ...
      const { error } = await supabase.auth.resend({
        ...
        options: {
          emailRedirectTo:
            supabaseAuthService.getEmailVerificationRedirectUrl("verification"),
        },
      });
  ...
  checkUserRegistrationStatus: async (userId: string) => {
    try {
      ...
      const hasCompletedOnboarding =
        hasProfile && profileData.has_completed_onboarding;
      const hasTravelPreferences = !!travelData;

      // Determine registration status
      let registrationStatus: UserRegistrationStatus =
        UserRegistrationStatus.RETURNING_USER;
