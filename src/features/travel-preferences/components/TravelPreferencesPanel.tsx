...
const travelPreferencesSchema = z.object({
  ...
  accommodationTypes: z
    .array(z.enum(["hotel", "apartment", "hostel", "resort"]))
...
const SimpleCheckboxCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div
        className={`h-4 w-4 rounded border ${selected ? "bg-planora-accent-purple border-planora-accent-purple" : "border-gray-300"} flex items-center justify-center`}
      >
...
const TravelPreferencesPanel: React.FC<TravelPreferencesPanelProps> = ({
    ...
    if (preferences && !isLoading) {
      try {
        ...
        const formattedPreferences: TravelPreferencesFormValues = {
          budgetRange: {
            min: Number(
              preferences.budgetRange && preferences.budgetRange.min
                ? preferences.budgetRange.min
                : 500,
            ),
            max: Number(
              preferences.budgetRange && preferences.budgetRange.max
          ...
          accommodationTypes: Array.isArray(preferences.accommodationTypes)
            ? (preferences.accommodationTypes.filter((type) =>
                ["hotel", "apartment", "hostel", "resort"].includes(
                  type as string,
                ),
              ) as AccommodationType[])
            : ["hotel"],
          accommodationComfort: Array.isArray(preferences.accommodationComfort)
            ? (preferences.accommodationComfort.filter((pref) =>
                [
                  "private-room",
                  "shared-room",
                  "private-bathroom",
          ...
          preferCheaperWithStopover:
            preferences.preferCheaperWithStopover === undefined
              ? true
              : preferences.preferCheaperWithStopover,
        };
  ...
  const onSubmit = async (formData: TravelPreferencesFormValues) => {
    ...
    try {
      ...
      const shouldClearCustomDateFlexibility =
        formData.travelDuration !== "longer";
      ...
      const completeFormValues = {
        ...
        // Clear customDateFlexibility if not needed
        customDateFlexibility: shouldClearCustomDateFlexibility
          ? ""
          : (formData.customDateFlexibility ?? ""),
        ...
        accommodationTypes: Array.isArray(formData.accommodationTypes)
          ? (formData.accommodationTypes.filter((type) =>
              Object.values(AccommodationType).includes(
        ...
        accommodationComfort: Array.isArray(formData.accommodationComfort)
          ? (formData.accommodationComfort.filter((pref) =>
              Object.values(ComfortPreference).includes(
                pref as ComfortPreference,
              ),
            ) as ComfortPreference[])
                      ...
                      <FormControl>
                        ...
                        >
                          <div className="p-3 border rounded-md flex items-start space-x-3">
                            ...
                            <div>
                              <label
                                htmlFor="exploring-intent"
                                className="font-medium cursor-pointer"
                              >
                                Just exploring ideas
                              </label>
                              <p className="text-sm text-muted-foreground mt-1">
                                Gathering inspiration for future travel
                                possibilities
                              </p>
                            </div>
                          </div>
                      ...
                      <FormItem>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {[
                            ...
                            // Type assertion for the string comparison
                            const isSelected = values.includes(
                              option.value as ComfortPreference,
                            );
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        ...
                        >
                          <div
                            className={`p-2 border rounded-md text-center ${field.value === "center" ? "border-planora-accent-purple bg-planora-accent-purple/10" : "border-white/10 bg-white/5"}`}
                          >
                          ...
                          <div
                            className={`p-2 border rounded-md text-center ${field.value === "near" ? "border-planora-accent-purple bg-planora-accent-purple/10" : "border-white/10 bg-white/5"}`}
                          >
                          ...
                          <div
                            className={`p-2 border rounded-md text-center ${field.value === "outskirts" ? "border-planora-accent-purple bg-planora-accent-purple/10" : "border-white/10 bg-white/5"}`}
                          >
                            <RadioGroupItem
                              value="outskirts"
                              id="outskirts-loc"
                          ...
                          <div
                            className={`p-2 border rounded-md text-center ${field.value === "anywhere" ? "border-planora-accent-purple bg-planora-accent-purple/10" : "border-white/10 bg-white/5"}`}
                          >
                            <RadioGroupItem
                              value="anywhere"
                              id="anywhere-loc"
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          ...
                          >
                            <div
                              className={`p-2 border rounded-md text-center ${field.value === "direct" ? "border-planora-accent-purple bg-planora-accent-purple/10" : "border-white/10 bg-white/5"}`}
                            >
                              <RadioGroupItem
                                value="direct"
                                id="direct-flight"
                            ...
                            <div
                              className={`p-2 border rounded-md text-center ${field.value === "any" ? "border-planora-accent-purple bg-planora-accent-purple/10" : "border-white/10 bg-white/5"}`}
                            >
                      ...
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                        ...
                        <div className="space-y-1 leading-none">
                          ...
                          <p className="text-sm text-muted-foreground">
                            We'll show you flights with stopovers if they're
                            more than 25% cheaper
                          </p>
                        </div>
                      </FormItem>
                    )}
                  ...
                  <div className="p-3 rounded-md border border-white/10 bg-white/5">
                    ...
                    <p className="font-medium">
                      ...
                      form.watch("departureCity")
                        ? `${form.watch("departureCountry")}, ${
                            form.watch("departureCity") === "Other"
                              ? form.watch("customDepartureCity") ||
                                "Custom City"
                              : form.watch("departureCity")
                          }`
