  ...
  const savePreferences = async (
    ...
    try {
      ...
      const completePreferences: TravelPreferences = {
        ...
        dateFlexibility:
          data.dateFlexibility || DateFlexibilityType.FLEXIBLE_WEEK,
        ...
        locationPreference:
          data.locationPreference || LocationPreference.CENTER,
      ...
      await travelPreferencesService.saveTravelPreferences(
        currentUser.id,
        completePreferences,
      );

      // Mark onboarding as completed
      await travelPreferencesService.updateOnboardingStatus(
        currentUser.id,
        true,
      );
      ...
      setError(
        new Error(error.message || "An error occurred loading preferences"),
      );
      throw error;
    } finally {
      setLoading(false);
