...
const DeleteAccountDialog: React.FC<DeleteAccountDialogProps> = ({
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      const { success, error } =
        await userProfileService.initiateAccountDeletion();

      if (success) {
        toast({
          title: "Account Deletion Initiated",
          description:
            "Your account deletion has been scheduled. You will receive an email with instructions on how to cancel this process if you change your mind.",
          duration: 8000,
        });
        ...
        const errorMessage =
          (error as Error & { context?: { function_error?: string } })?.context
            ?.function_error ||
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            This action will schedule your account for permanent deletion in 30
            days. You will receive an email with a link to cancel this process.
          </DialogDescription>
        </DialogHeader>
