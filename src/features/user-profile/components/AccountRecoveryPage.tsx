  ...
  useEffect(() => {
    const processRecoveryToken = async () => {
      try {
        ...
        if (!token) {
          setError('No recovery token found in the URL. Please check your recovery link.');
        ...
        if (result.success) {
          ...
          setError(result.error || 'Failed to recover your account. The recovery link may have expired.');
        }
      } catch (error) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-50 p-4">
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle>
            {isProcessing ? 'Processing Recovery...' : 
             isSuccess ? 'Account Recovered!' : 
             'Account Recovery Failed'}
          </CardTitle>
          <CardDescription>
            {isProcessing ? 'Please wait while we process your request...' :
             isSuccess ? 'Your account has been successfully recovered.' :
        ...
        <CardContent>
          {isProcessing ? (
            ...
            <div className="bg-green-50 border border-green-200 rounded-md p-4 text-green-800">
              <p className="font-medium">Your account has been successfully recovered!</p>
              <p className="mt-2">Your account is now active again. You can sign in using your existing credentials.</p>
            </div>
          ) : (
