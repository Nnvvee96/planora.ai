...
export function DatePickerInput({ value, onChange, placeholder = "MM / DD / YYYY", className }: DatePickerInputProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newValue.length > 0) {
      ...
      if (newValue.length >= 1) {
        ...
        if (monthNum > 12) {
          monthNum = 12;
          newValue = monthNum.toString().padStart(2, '0') + newValue.substring(Math.min(2, newValue.length));
        }
      }
      ...
      if (newValue.length > 2) {
        ...
        if ([4, 6, 9, 11].includes(monthNum)) {
          ...
          const yearPart = newValue.length >= 5 ? parseInt(newValue.substring(4, 8), 10) : new Date().getFullYear();
          maxDay = ((yearPart % 4 === 0 && yearPart % 100 !== 0) || yearPart % 400 === 0) ? 29 : 28;
        }
      ...
      if (formatted.length === 10) { // MM/DD/YYYY format is 10 chars
