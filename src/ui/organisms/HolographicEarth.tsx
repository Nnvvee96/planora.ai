...
const HolographicEarth = () => {
  ...
  useFrame((state) => {
    ...
    if (continentParticlesRef.current) {
      const material = continentParticlesRef.current.material as THREE.PointsMaterial;
      material.opacity = 0.9 + Math.sin(time * 1.5) * 0.1;
    }
    
    if (oceanParticlesRef.current) {
      const material = oceanParticlesRef.current.material as THREE.PointsMaterial;
