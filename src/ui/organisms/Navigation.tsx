...
const navItems = [
  ...
  { id: 'features', label: 'Features', type: 'scroll', sectionId: 'features', path: '/#features' },
  { id: 'how-it-works', label: 'How It Works', type: 'scroll', sectionId: 'how-it-works', path: '/#how-it-works' },
  { id: 'examples', label: 'Examples', type: 'scroll', sectionId: 'examples', path: '/#examples' },
  { id: 'pricing', label: 'Pricing', type: 'scroll', sectionId: 'pricing', path: '/#pricing' },
  { id: 'reviews-landing', label: 'Reviews', type: 'scroll', sectionId: 'user-stories', path: '/#user-stories' }, // For landing page section
...
const Navigation: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero', 'features', 'how-it-works', 'examples', 'pricing', 'tech', 'faq', 'user-stories', 'cta'];
      ...
      const currentPageNavItem = navItems.find(item => item.type === 'page' && item.path === location.pathname);
      if (currentPageNavItem) {
        setActiveSection(currentPageNavItem.id);
      } else if (location.pathname === '/') { // Only apply scroll-based active section on landing page
  ...
  const scrollToSection = (id: string) => {
    ...
    if (element) {
      ...
      if (navItem && navItem.path && window.location.pathname !== navItem.path.split('#')[0]) {
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/60 backdrop-blur-xl shadow-lg shadow-planora-accent-purple/5' : 'bg-transparent'}`}>
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center space-x-1"> {/* Reduced space for potentially more items */}
            {/* Modern nav items with indicator for active section */}
            {navItems.filter(item => item.label !== 'All Reviews').map((item) => { // Exclude 'All Reviews' from main nav, or decide its place
              const isActive = activeSection === item.id || (item.id === 'home' && activeSection === 'hero') || (item.id === 'reviews-page' && location.pathname === '/reviews');
              if (item.type === 'page') {
                return (
                  <Link
                    key={item.id}
            ...
            <div className="flex flex-col space-y-1 px-2">
              {navItems.map((item, index) => {
                const isActive = (
                  ...
                  (item.type === 'scroll' && activeSection === (item.sectionId || item.id)) || 
                  (item.id === 'home' && activeSection === 'hero' && location.pathname === '/')
                ); // Special case for home on landing
                ...
                return (
                  <button 
                    key={item.id}
                    onClick={() => { scrollToSection(item.sectionId || item.id); setIsMenuOpen(false); }} 
                    className={`text-left p-3 rounded-lg transition-all duration-300 flex items-center justify-between
                      ${isActive 
                        ? 'bg-gradient-to-r from-planora-accent-purple/20 to-planora-accent-pink/10 text-white' 
                        : 'text-white/70 hover:bg-white/5 hover:text-white'}`}
